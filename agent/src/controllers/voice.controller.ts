...
export class VoiceConnectionHandler {
  ...
  private onMessage(data: Buffer | string): void {
    ...
    const payload = parseTextMessage(data) as
      { type?: string; text?: string; sentence?: string } | string | null;
    if (!payload) return;

    if (typeof payload === 'object' && payload.type === 'INTERRUPT') {
  ...
  private sendProcessingStart(): void {
    if (this.ws.readyState === WebSocket.OPEN) {
      console.log(
        `[VoiceConnection] Sending AI_PROCESSING_START to client for conversation: ${this.conversationId}`
      );
  ...
  private sendTurnComplete(): void {
    if (this.ws.readyState === WebSocket.OPEN) {
      console.log(
        `[VoiceConnection] Sending AI_TURN_COMPLETE to client for conversation: ${this.conversationId}`
      );
  ...
  private onError(error: Error): void {
    console.error(
      `[VoiceConnection] WebSocket error for conversation ${this.conversationId}:`,
      error
    );
    voiceService.endSession(this.conversationId);
    this.ws.removeAllListeners();
