...
export function Message({ from, children, className }: { from?: string; children: React.ReactNode; className?: string }) {
    const isUser = from === 'user';
    return <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} ${className}`}>{children}</div>;
}
