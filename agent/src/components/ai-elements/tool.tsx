...
export function Tool({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div
      className={`my-4 rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-900 ${className}`}
    >
      {children}
    </div>
  );
...
export function ToolHeader({
  ...
  return (
    <div
      className={`mb-2 flex items-center justify-between font-medium text-gray-700 dark:text-gray-300 ${className}`}
    >
      🛠️ {toolName} ({state})
    </div>
  );
...
export function ToolInput({ input }: { input: Record<string, unknown> }) {
  return (
    <div className="text-sm">
      ...
      <pre className="mt-1 overflow-x-auto rounded bg-gray-200 p-2 text-xs dark:bg-gray-800">
