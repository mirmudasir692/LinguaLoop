...
interface ISpeechRecognition {
  ...
  onerror: ((err: ISpeechRecognitionErrorEvent) => void) | null;
  onend: (() => void) | null;
  start: () => void;
  abort: () => void;
  stop: () => void;
...
export function useVoiceChat(
  ...
  const [aiTranscript, _setAiTranscript] = useState<string>('');
      ...
      try {
        ...
        if (voicesData && isMounted) {
          const voicesList: Voice[] = Array.isArray(voicesData)
            ? (voicesData as Voice[])
            : ((voicesData as Record<string, unknown>).voices as Voice[]) || [];
  ...
  const isBraveBrowser = useCallback(async (): Promise<boolean> => {
    try {
      ...
      return Boolean(
        nav.brave && typeof nav.brave.isBrave === 'function' && (await nav.brave.isBrave())
      );
    } catch {
      return false;
    }
  ...
  const muteAndSendUserSpeech = useCallback(
    (sentence: string) => {
      ...
      console.log(
        `[VoiceChat] User sentence finalized after 3s pause. Sent to server: "${finalSentence}" (Muting microphone)`
      );
      ...
      watchdogTimeoutRef.current = setTimeout(() => {
        console.warn(
          '[VoiceChat] Watchdog timeout: auto-unmuting microphone after processing inactivity'
        );
        audioQueueRef.current = [];
        isPlayingAudioRef.current = false;
        isTurnCompleteRef.current = true;
  ...
  const startListening = useCallback(() => {
    if (!isCallActiveRef.current || isAiSpeakingRef.current || isProcessingRef.current) {
      console.log(
        '[VoiceChat] startListening skipped (active:',
    ...
    if (!SpeechRec) {
      setVoiceError(
        'Web Speech API is not supported in this browser. Please use Google Chrome, Edge, or Safari.'
      );
      return;
    }
    ...
    try {
      ...
      recognition.onstart = () => {
        console.log(
          '[VoiceChat] Speech recognition listening started (continuous: true, 3s pause detection active)'
        );
      };
      ...
      recognition.onresult = (event: ISpeechRecognitionEvent) => {
        ...
        const fullText =
          (baseTranscriptRef.current ? baseTranscriptRef.current + ' ' : '') + currentSessionText;
        ...
        if (trimmedFullText) {
          ...
          silenceTimerRef.current = setTimeout(() => {
            ...
            if (candidate && !isProcessingRef.current && !isAiSpeakingRef.current) {
              console.log(
                `[VoiceChat] 3-second silence detected. Finalizing user sentence: "${candidate}"`
              );
      ...
      recognition.onerror = async (err: ISpeechRecognitionErrorEvent) => {
      ...
      recognition.onend = () => {
        ...
        if (isCallActiveRef.current && !isAiSpeakingRef.current && !isProcessingRef.current) {
          if (consecutiveNetworkErrorsRef.current >= 3) {
            console.warn(
              '[VoiceChat] Auto-restart paused due to persistent network errors. Click Retry to reconnect.'
            );
            return;
          }
    ...
    (voiceId?: string) => {
      ...
      ws.onmessage = (event) => {
        ...
        if (event.data instanceof ArrayBuffer || event.data instanceof Blob) {
          const audioBlob =
            event.data instanceof Blob ? event.data : new Blob([event.data], { type: 'audio/mp3' });
  ...
  const startCall = useCallback(async () => {
    try {
      ...
      if (!SpeechRec) {
        setVoiceError(
          'Your browser does not support Web Speech API. Please use Google Chrome, Edge, or Safari.'
        );
        return;
      }
      ...
      setVoiceError(`Failed to start call: ${errObj?.message || error}`);
    }
